...
class PidController {
 ...
 public:
  ...
  PidController(double Param_kp, double Param_ki, double Param_kd,
                double default_currentSpeed, double default_currentTime,
                double default_desiredSpeed)
      :
  ...
  ~PidController() {
  } /*Default destructor for PidController class*/

  /**Calculate controller gain*/
  double calculateGain(const double currentSpeed, const double currentTime);
